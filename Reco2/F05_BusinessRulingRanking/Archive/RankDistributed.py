...
if var_country_code in ['US']:
  available_product_images_acm = spark.table('data_config_all.available_product_images_acm_updated')

if var_country_code not in ['US']:
...
product_cycle_filter = spark.table('data_config_emea.es_product_details').where(col('productCode').isNotNull()).where(lower(col('ActivationCycleC2'))== 'yes')
recipe_cycle_filter = spark.table('data_config_emea.es_recipe_details').where(col('recipeId').isNotNull()).where(lower(col('C2')) == 'yes')

lst_product_cycle_filter = product_cycle_filter.select('cuEanCodes').distinct().rdd.map(lambda r: r[0]).collect()
lst_recipe_cycle_filter = recipe_cycle_filter.select('recipeId').distinct().rdd.map(lambda r: r[0]).collect()

print(lst_product_cycle_filter)
print(lst_recipe_cycle_filter)

# COMMAND ----------

recommendation_set.show()

# COMMAND ----------

# Cases with Invalid local channel
recommendation_set_complete = recommendation_set.where(recommendation_set.productCode != '').where(recommendation_set.recipeId != '')

recos_addition = recommendation_set_complete.select('productCode', 'cuEanCodes', 'productCodeLocal', 'recipeId').distinct()

if var_country_code in ['US']:
  recos_addition = recos_addition.where(col("productCodeLocal").isin(lst_available_images))

if var_country_code not in ['US']:
  recos_addition = recos_addition.where(col("cuEanCodes").isin(lst_available_images))  
  
if var_country_code in ['ES']:
  recos_addition = recos_addition.where(col("cuEanCodes").isin(lst_product_cycle_filter))
  recos_addition = recos_addition.where(col("recipeId").isin(lst_recipe_cycle_filter))

recos_addition.show()

recommendation_set_addition = recommendation_set.where(recommendation_set.productCode == '').distinct()
recommendation_set_addition.show()

recommendation_set_addition = recommendation_set_addition.select('operatorOhubId', 'Local_channel').distinct()
recommendation_set_addition = recommendation_set_addition.crossJoin(recos_addition)

# Add Reco Ids
w1 =  Window.partitionBy(recommendation_set_addition.operatorOhubId).orderBy(recommendation_set_addition.productCode.desc(), recommendation_set_addition.recipeId.desc())
recommendation_set_addition = recommendation_set_addition.withColumn('recoids', dense_rank().over(w1))


recommendation_set_addition = recommendation_set_addition.withColumn('Local_channel', concat(lit('No product fit - '),col('Local_channel')).cast('string'))

recommendation_set_addition = recommendation_set_addition.select('operatorOhubId', 'Local_channel', 'recoids', 'productCode', 'cuEanCodes', 'productCodeLocal', 'recipeId').distinct()

recommendation_set_addition.show()

recommendation_set = recommendation_set_complete.unionAll(recommendation_set_addition)
...
#recommendation_set.count()

# COMMAND ----------

#recommendation_set.withColumn('recipeId',\
#                                       when(col('recipeId').isNotNull(),col('recipeId')).\
#                                       when(col('recipeId').isNotNull(),lit(''))
#                                      ).select('operatorOhubId', 'productCode', 'recipeId').distinct().count()
...
if var_country_code in ['ES']:
  tmp0b_ods_recommendations = tmp0b_ods_recommendations.where(col("cuEanCodes").isin(lst_product_cycle_filter))
  tmp0b_ods_recommendations = tmp0b_ods_recommendations.where(col("recipeId").isin(lst_recipe_cycle_filter))
...
# MAGIC %sql
# MAGIC 
# MAGIC Select  cuEanCodes,
# MAGIC         --personalized,
# MAGIC         cast('Recos' as string) as entity,
# MAGIC         cast('AT' as string) as countryCode,
# MAGIC         count(*), 
# MAGIC         count(distinct operatorOhubId) as cnt_ops,
# MAGIC         count(distinct emailAddress) as cnt_email
# MAGIC From data_reco_output.ods_at_recommendations_test_20190602
# MAGIC Where Local_channel like '%No product fit%'
# MAGIC Group by cuEanCodes--,
# MAGIC         --personalized
# MAGIC ;
